...
返回：几何、全局 X/Y/Z 包围盒、车间长×宽×厚、几何最小毛坯、实际加工毛坯来源/局部坐标/方向、H2 原始刀路总工时、六阶段工时、孔加工/粗加工/精加工/倒角去毛刺四类 CNC 工时、一个三轴/车铣/五轴建议、适用时的一份机器学习三轴装夹次数及置信度、DFM、3D 预览；不返回平台价格、交期或内部定价信息。
...
说明：服务端只做制造分析。仅当建议为三轴且存在有效装夹次数时，Agent 才能使用本机 cost-profile 按固定公式计算；车铣或五轴建议不计算价格，费率不会上传。
