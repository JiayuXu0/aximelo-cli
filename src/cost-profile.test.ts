...
describe("local cost profile", () => {
  ...
  it("reports missing and saves the five initial rates with zero stock adjustment", async () => {
    ...
    expect(profile).toMatchObject({ currency: "CNY", startup_fee_per_design: 100, programming_fee_per_design: 200, machine_hour_rate: 0, setup_fee_per_setup: 0, materials: { "6061": { price_per_kg: 30 } }, stock_adjustment: { block_allowance_per_side_mm: 0, round_up_mm: 0 } });
  ...
  it("appends new materials and independently updates stock adjustments", async () => {
    ...
    expect(profile.stock_adjustment).toMatchObject({ block_allowance_per_side_mm: 0, cylinder_radial_allowance_mm: 2, cylinder_end_allowance_mm: 0, round_up_mm: 5 });
  });
